# PowerShell script to run discount codes seeding in Docker production container
# This copies the script into the container and runs it

Write-Host "🐳 Running discount codes seeding in Docker production container..." -ForegroundColor Cyan
...
docker cp scripts/seed-comprehensive-discount-codes.ts "${containerId}:/app/scripts/seed-comprehensive-discount-codes.ts"
