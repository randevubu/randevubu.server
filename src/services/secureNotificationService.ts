...
import { CustomerRelationshipService, CustomerValidationResult } from './customerRelationshipService';
...
export interface SecureNotificationRequest {
  businessId: string;
  userId: string; // User sending the notification
  recipientIds: string[];
  title: string;
  body: string;
...
export interface SecureNotificationResult {
  ...
  validationResults: CustomerValidationResult[];
  errors: Array<{
    recipientId: string;
    error: string;
    errorCode: string;
  ...
  async sendSecureNotification(
    ...
    try {
      ...
      await this.auditService.logBatchNotification(
        ...
        failedCount,
        {
          validRecipients: validationResult.validCount,
