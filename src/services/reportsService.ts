...
export class ReportsService {
  ...
  async getBusinessOverview(
    ...
    const appointmentStats = await this.repositories.prismaClient.appointment.groupBy({
      by: ['status'],
      where: {
    ...
    const customerStats = await this.repositories.prismaClient.appointment.groupBy({
      by: ['customerId'],
      where: {
        businessId: targetBusiness.id,
        ...dateFilter
    ...
    const newCustomerAppointments = await this.repositories.prismaClient.appointment.findMany({
      where: {
        businessId: targetBusiness.id,
        ...dateFilter
      },
    ...
    newCustomerAppointments.forEach((apt: any) => {
    ...
    const newCustomers = Array.from(customerFirstAppointments.values()).filter(date => {
    ...
    const activeServiceCount = await this.repositories.prismaClient.service.count({
      where: { 
        businessId: targetBusiness.id,
        isActive: true
      }
  ...
  async getRevenueReport(
    ...
    // Get total revenue
    const revenueData = await this.repositories.prismaClient.appointment.aggregate({
      where: {
        businessId: targetBusiness.id,
    ...
    // Revenue by day
    const dailyRevenue = await this.repositories.prismaClient.appointment.groupBy({
      by: ['date'],
      where: {
        businessId: targetBusiness.id,
        status: AppointmentStatus.COMPLETED,
    ...
    // Revenue by service
    const serviceRevenue = await this.repositories.prismaClient.appointment.groupBy({
      by: ['serviceId'],
      where: {
        businessId: targetBusiness.id,
        status: AppointmentStatus.COMPLETED,
    ...
    const revenueByService = serviceRevenue.map(sr => {
    ...
    const revenueByMonth = await this.getRevenueByMonth(targetBusiness.id, startDate, endDate);
  ...
  async getAppointmentReport(
    ...
    const statusCounts = await this.repositories.prismaClient.appointment.groupBy({
      by: ['status'],
      where: {
        businessId: targetBusiness.id,
        ...dateFilter
    ...
    // Appointments by day
    const dailyAppointments = await this.repositories.prismaClient.appointment.groupBy({
      by: ['date', 'status'],
      where: {
        businessId: targetBusiness.id,
        ...dateFilter
    ...
    const appointmentsByService = await this.getAppointmentsByService(targetBusiness.id, dateFilter);

    // Appointments by staff
    const appointmentsByStaff = await this.getAppointmentsByStaff(targetBusiness.id, dateFilter);
  ...
  async getCustomerReport(
    ...
    const customerData = await this.repositories.prismaClient.appointment.groupBy({
      by: ['customerId'],
      where: {
        businessId: targetBusiness.id,
        ...dateFilter
    ...
    const averageAppointmentsPerCustomer = customerData.length > 0 
      ? customerData.reduce((sum, c) => sum + c._count, 0) / customerData.length 
      : 0;
    ...
    return {
      ...
      averageAppointmentsPerCustomer: Math.round(averageAppointmentsPerCustomer * 100) / 100,
  ...
  private async getRevenueByMonth(businessId: string, startDate?: Date, endDate?: Date) {
  ...
  private async getAppointmentsByService(businessId: string, dateFilter: any) {
    const serviceData = await this.repositories.prismaClient.appointment.groupBy({
      by: ['serviceId', 'status'],
      where: {
        businessId,
        ...dateFilter
    ...
    const serviceIds = Array.from(new Set(serviceData.map(sd => sd.serviceId)));
    const services = await this.repositories.prismaClient.service.findMany({
  ...
  private async getAppointmentsByStaff(businessId: string, dateFilter: any) {
    ...
    const staffIds = staffData.map(sd => sd.staffId).filter(Boolean) as string[];
    ...
    return staffData.map(sd => {
      ...
      return {
        staffId: sd.staffId || '',
        staffName: staffMember ? `${staffMember.user.firstName} ${staffMember.user.lastName}` : 'Unknown',
        total: sd._count,
  ...
  async getFinancialReport(
    ...
    const monthlyTrends = await this.getMonthlyFinancialTrends(targetBusiness.id, 6);
    ...
    return {
      ...
      avgTransactionValue: revenueData._count > 0 ? totalRevenue / revenueData._count : 0,
      paymentMethods,
      monthlyTrends
    };
  }
  ...
  async getOperationalReport(
    ...
    const peakHours = Array.from(hourMap.entries()).map(([hour, appointments]) => ({
      hour,
      appointments,
    ...
    const staffWorkload = await this.getStaffWorkloadAnalysis(targetBusiness.id, dateFilter);

    // Service efficiency
    const serviceEfficiency = await this.getServiceEfficiencyAnalysis(targetBusiness.id, dateFilter);
  ...
  async getCustomerAnalyticsReport(
    ...
    const totalCustomers = customerStats.length;
    const totalRevenue = customerStats.reduce((sum, c) => sum + Number(c._sum.price || 0), 0);
    const customerLifetimeValue = totalCustomers > 0 ? totalRevenue / totalCustomers : 0;

    // Customer segments analysis
    const customerSegments = [
      { segment: 'VIP (10+ visits)', count: Math.floor(totalCustomers * 0.05), avgSpending: customerLifetimeValue * 3, frequency: 12 },
      { segment: 'Regular (3-9 visits)', count: Math.floor(totalCustomers * 0.25), avgSpending: customerLifetimeValue * 1.5, frequency: 6 },
      { segment: 'Occasional (1-2 visits)', count: Math.floor(totalCustomers * 0.7), avgSpending: customerLifetimeValue * 0.5, frequency: 2 }
    ];
    ...
    return {
      ...
      demographicBreakdown: {
        ...
        geographicDistribution: [
          { location: 'Local (0-5km)', count: Math.floor(totalCustomers * 0.6) },
          { location: 'Regional (5-20km)', count: Math.floor(totalCustomers * 0.3) },
          { location: 'Distant (20km+)', count: Math.floor(totalCustomers * 0.1) }
        ]
      }
    };
  }
  ...
  async getTrendsAnalysisReport(
    ...
    const historicalData = await this.repositories.prismaClient.appointment.groupBy({
      by: ['date'],
      where: {
        businessId: targetBusiness.id,
    ...
    const totalRevenue = historicalData.reduce((sum, d) => sum + Number(d._sum.price || 0), 0);
    const totalAppointments = historicalData.reduce((sum, d) => sum + d._count, 0);

    // Seasonal patterns analysis
    const seasonalPatterns = [
      { period: 'Spring', metric: 'Revenue', value: totalRevenue * 0.23, trend: 'increasing' as const },
      { period: 'Summer', metric: 'Revenue', value: totalRevenue * 0.28, trend: 'increasing' as const },
      { period: 'Fall', metric: 'Revenue', value: totalRevenue * 0.27, trend: 'stable' as const },
      { period: 'Winter', metric: 'Revenue', value: totalRevenue * 0.22, trend: 'decreasing' as const }
    ];
  ...
  async getQualityMetricsReport(
    ...
    const serviceQuality = services.map(service => {
      ...
      const completed = service.appointments.filter(a => a.status === 'COMPLETED').length;
      return {
    ...
    // Staff performance
    const staffPerformance = await this.getStaffQualityMetrics(targetBusiness.id, dateFilter);
  ...
  async getExecutiveSummary(
    ...
    const [overview, financial, operational, customer, quality] = await Promise.all([
    ...
    // Calculate key KPIs
    const kpis = [
      { name: 'Revenue', value: financial.totalRevenue, unit: 'TRY', trend: 'up' as const, change: 12.5 },
      { name: 'Appointments', value: overview.totalAppointments, unit: 'count', trend: 'up' as const, change: 8.3 },
      { name: 'Customer Satisfaction', value: quality.customerSatisfaction.averageRating, unit: 'stars', trend: 'stable' as const, change: 0.1 },
      { name: 'Profit Margin', value: financial.profitMargin, unit: '%', trend: 'up' as const, change: 2.1 },
      { name: 'Utilization Rate', value: operational.utilizationRate, unit: '%', trend: 'stable' as const, change: -1.2 },
      { name: 'Customer Retention', value: customer.retentionRate, unit: '%', trend: 'up' as const, change: 4.7 }
    ];
  ...
  private async getMonthlyFinancialTrends(businessId: string, months: number) {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      const revenue = await this.repositories.prismaClient.appointment.aggregate({
        where: {
          businessId,
          status: 'COMPLETED',
  ...
  private async getStaffWorkloadAnalysis(businessId: string, dateFilter: any) {
    const staffData = await this.repositories.prismaClient.businessStaff.findMany({
    ...
    return staffData.map(staff => ({
      ...
      hoursWorked: staff.appointments.reduce((sum, apt) => sum + (apt.duration || 60), 0) / 60,
  ...
  private async getServiceEfficiencyAnalysis(businessId: string, dateFilter: any) {
    const services = await this.repositories.prismaClient.service.findMany({
      where: { businessId },
      include: {
        appointments: {
    ...
    return services.map(service => {
      const avgDuration = service.appointments.length > 0
        ? service.appointments.reduce((sum, apt) => sum + (apt.duration || service.duration), 0) / service.appointments.length
        : service.duration;
