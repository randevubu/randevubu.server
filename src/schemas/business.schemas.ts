...
export const createBusinessHoursOverrideSchema = z
  .object({
    date: z
      .string()
...
export const updateBusinessPriceSettingsSchema = z.object({
  hideAllServicePrices: z
    .boolean()
    .optional()
    .describe("Hide prices for all services business-wide"),

  showPriceOnBooking: z
    .boolean()
    .optional()
    .default(true)
    .describe(
      "Show prices during the booking flow even if hidden on service list"
    ),

  priceDisplayMessage: z
    .string()
    .max(100, "Price display message must be less than 100 characters")
    .optional()
    .describe(
      'Custom message to show when prices are hidden (e.g., "Contact us for pricing")'
    ),
});
...
export const updateBusinessStaffPrivacySettingsSchema = z.object({
  hideStaffNames: z
    ...
    .default(false)
    .describe(
      "Hide individual staff member names from customers during booking"
    ),
  ...
  staffDisplayMode: z
    ...
    .default("NAMES")
    .describe(
      "How to display staff to customers: NAMES (show actual names), ROLES (show role titles), GENERIC (show generic labels)"
    ),
...
export const createBusinessClosureSchema = z
  .object({
...
export const changePlanSchema = z.object({
  ...
  prorationPreference: z.enum(["prorate", "credit_forward"]).default("prorate"),
});
...
// Query parameters validation for appointment endpoints
...
export const validateClosureDates = (
  startDate: string,
  endDate?: string
): boolean => {
  try {
...
export const imageUploadSchema = z.object({
  imageType: z.enum(["logo", "cover", "profile", "gallery"], {
    errorMap: () => ({
      message: "Image type must be one of: logo, cover, profile, gallery",
    }),
  }),
});
...
export const deleteImageSchema = z.object({
  imageType: z.enum(["logo", "cover", "profile"], {
    errorMap: () => ({
      message: "Image type must be one of: logo, cover, profile",
    }),
  }),
});
...
export type UpdateBusinessPriceSettingsSchema = z.infer<
  typeof updateBusinessPriceSettingsSchema
>;
export type UpdateBusinessStaffPrivacySettingsSchema = z.infer<
  typeof updateBusinessStaffPrivacySettingsSchema
>;
...
export type CreateBusinessClosureSchema = z.infer<
  typeof createBusinessClosureSchema
>;
export type UpdateBusinessClosureSchema = z.infer<
  typeof updateBusinessClosureSchema
>;
...
export type AppointmentQuerySchema = z.infer<typeof appointmentQuerySchema>;

// Image schema types
export type ImageUploadSchema = z.infer<typeof imageUploadSchema>;
export type UpdateBusinessImagesSchema = z.infer<
  typeof updateBusinessImagesSchema
>;
...
export type CreateBusinessHoursOverrideSchema = z.infer<
  typeof createBusinessHoursOverrideSchema
>;
export type UpdateBusinessHoursOverrideSchema = z.infer<
  typeof updateBusinessHoursOverrideSchema
>;
export type BusinessHoursStatusSchema = z.infer<
  typeof businessHoursStatusSchema
>;

// Business Notification Settings schemas
export const businessNotificationSettingsSchema = z
  .object({
  ...
  .refine(
    (data) => {
      ...
      if (data.quietHours) {
        const [startHour, startMin] = data.quietHours.start
          .split(":")
          .map(Number);
...
export const updateBusinessNotificationSettingsSchema = z.object({
  ...
  reminderChannels: z
    ...
    .optional()
    .describe(
      "Notification channels to use for reminders (auto-synced with enabled channels)"
    ),
  ...
  smsEnabled: z.boolean().optional().describe("Enable SMS notifications"),
...
export type BusinessNotificationSettingsSchema = z.infer<
  typeof businessNotificationSettingsSchema
>;
export type UpdateBusinessNotificationSettingsSchema = z.infer<
