...
import { BaseError, InternalServerError } from '../types/errors';
...
export class ReportsController {
   ...
   */
  getBusinessOverview = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (error instanceof Error && error.message.includes('No accessible businesses')) {
        res.status(403).json({
          success: false,
          error: {
   ...
   */
  getRevenueReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    try {
      const userId = req.user.id;
      const { businessId, startDate, endDate } = req.query;
  ...
  exportReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!['overview', 'revenue', 'appointments', 'customers'].includes(reportType)) {
        res.status(400).json({
          success: false,
          error: {
      ...
      switch (reportType) {
        case 'overview':
          report = await this.reportsService.getBusinessOverview(userId, businessId as string, start, end);
          break;
        case 'revenue':
          report = await this.reportsService.getRevenueReport(userId, businessId as string, start, end);
          break;
        case 'appointments':
          report = await this.reportsService.getAppointmentReport(userId, businessId as string, start, end);
          break;
        case 'customers':
          report = await this.reportsService.getCustomerReport(userId, businessId as string, start, end);
          break;
      }
  ...
  getBusinessComparison = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    try {
      ...
      this.handleError(error, res, 'Failed to retrieve business comparison report');
    }
  };
  ...
  private overviewToCSV(data: any): string {
    const headers = [
      'Business Name', 'Total Appointments', 'Completed', 'Canceled', 'No Show',
      'Total Revenue', 'Average Value', 'Completion Rate %', 'Total Customers'
    ];
  ...
  private customersToCSV(data: any): string {
    ...
    const rows = data.topCustomers.map((customer: any) => 
      [customer.customerName, customer.totalAppointments, customer.totalSpent, customer.reliabilityScore].join(',')
    );
  ...
  getCustomerAnalyticsReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    try {
      ...
      this.handleError(error, res, 'Failed to retrieve customer analytics report');
    }
  };
  ...
  generateCustomReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const validReportTypes = ['overview', 'revenue', 'appointments', 'customers', 'financial', 'operational'];
    ...
    try {
      ...
      if (!reportType || !name || !schedule || !recipients?.length) {
        res.status(400).json({
          success: false,
          error: {
            message: 'Missing required fields: reportType, name, schedule, recipients',
