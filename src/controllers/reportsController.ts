...
export class ReportsController {
  ...
  getBusinessOverview = async (req: ReportsRequest, res: Response): Promise<void> => {
    ...
    if (startDate && start && isNaN(start.getTime())) {
      throw new AppError('INVALID_DATE_FORMAT', { message: 'Invalid start date format' });
    }

    if (endDate && end && isNaN(end.getTime())) {
      throw new AppError('INVALID_DATE_FORMAT', { message: 'Invalid end date format' });
    }
  ...
  getDashboardReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    await this.responseHelper.success(
      res,
      'success.report.dashboardRetrieved',
      {
        overview,
        revenue,
        appointments,
  ...
  exportReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    if (!reportType || typeof reportType !== 'string') {
      throw new AppError('REQUIRED_FIELD_MISSING', { message: 'Report type is required', params: { field: 'reportType' } });
    }

    const validReportTypes = ['overview', 'revenue', 'appointments', 'customers'];
    if (!validReportTypes.includes(reportType)) {
      throw new AppError('VALIDATION_ERROR', { message: 'Invalid report type. Must be one of: overview, revenue, appointments, customers' });
    }
    ...
    if (format && !['json', 'csv'].includes(format as string)) {
      throw new AppError('VALIDATION_ERROR', { message: 'Invalid format. Must be json or csv' });
    }
  ...
  getBusinessComparison = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    if (businesses.length < 2) {
      throw new AppError('VALIDATION_ERROR', { message: 'Need at least 2 businesses for comparison' });
    }
    ...
    await this.responseHelper.success(
      res,
      'success.report.comparisonRetrieved',
      {
        comparisons,
  ...
  getFinancialReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    await this.responseHelper.success(res, 'success.report.financialRetrieved', report, 200, req);
  };
  ...
  getOperationalReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    await this.responseHelper.success(res, 'success.report.operationalRetrieved', report, 200, req);
  };
  ...
  getCustomerAnalyticsReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    await this.responseHelper.success(res, 'success.report.customerAnalyticsRetrieved', report, 200, req);
  };
  ...
  getTrendsAnalysisReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    await this.responseHelper.success(res, 'success.report.trendsRetrieved', report, 200, req);
  };
  ...
  getQualityMetricsReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    await this.responseHelper.success(res, 'success.report.qualityRetrieved', report, 200, req);
  };
  ...
  getExecutiveSummary = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    await this.responseHelper.success(
      res,
      'success.report.executiveSummaryRetrieved',
      {
        ...report,
        generatedAt: new Date().toISOString(),
        reportPeriod: {
  ...
  getRealtimeMetrics = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    await this.responseHelper.success(res, 'success.report.realtimeRetrieved', metrics, 200, req);
  };
  ...
  generateCustomReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    if (!reportType || !businessId) {
      throw new AppError('REQUIRED_FIELD_MISSING', { message: 'Report type and business ID are required', params: { field: 'reportType,businessId' } });
    }
    ...
    if (!validReportTypes.includes(reportType)) {
      throw new AppError('VALIDATION_ERROR', { message: 'Invalid report type. Must be one of: overview, revenue, appointments, customers, financial, operational' });
    }
    ...
    if (groupBy && !validGroupBy.includes(groupBy)) {
      throw new AppError('VALIDATION_ERROR', { message: 'Invalid groupBy value. Must be one of: day, week, month, year' });
    }
  ...
  getReportTemplates = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    const templates = [
      ...
      await this.responseHelper.success(
        res,
        'success.report.templatesRetrieved',
        {
          templates,
  ...
  scheduleReport = async (req: GuaranteedAuthRequest, res: Response): Promise<void> => {
    ...
    if (!reportType || !name || !schedule || !recipients?.length) {
      throw new AppError('REQUIRED_FIELD_MISSING', { message: 'Missing required fields: reportType, name, schedule, recipients', params: { field: 'reportType,name,schedule,recipients' } });
    }
    ...
    if (name.length < 3 || name.length > 100) {
      throw new AppError('VALIDATION_ERROR', { message: 'Name must be between 3 and 100 characters' });
    }

    // Validate format
    const validFormats = ['pdf', 'csv', 'json'];
    if (format && !validFormats.includes(format)) {
      throw new AppError('VALIDATION_ERROR', { message: 'Invalid format. Must be one of: pdf, csv, json' });
    }

    // Validate schedule structure
    if (!schedule.frequency || !['daily', 'weekly', 'monthly'].includes(schedule.frequency)) {
      throw new AppError('VALIDATION_ERROR', { message: 'Invalid schedule frequency. Must be daily, weekly, or monthly' });
    }

    // Validate recipients
    if (!Array.isArray(recipients) || recipients.length === 0) {
      throw new AppError('VALIDATION_ERROR', { message: 'Recipients must be a non-empty array' });
    }
    ...
    for (const recipient of recipients) {
      if (typeof recipient !== 'string' || !emailRegex.test(recipient)) {
        throw new AppError('VALIDATION_ERROR', { message: 'Invalid email format in recipients' });
      }
    }
