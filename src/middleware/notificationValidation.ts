...
import { ValidationErrors } from '../utils/errorResponse';
 ...
 */
async function validateBusinessRules(settings: any, businessId: string): Promise<void> {
  // Ensure at least one channel is enabled if appointment reminders are enabled
  if (settings.enableAppointmentReminders && settings.reminderChannels.length === 0) {
    throw new Error('At least one reminder channel must be selected when appointment reminders are enabled');
  }
  ...
  const invalidChannels = settings.reminderChannels.filter((channel: string) => 
    !enabledChannels.includes(channel)
  );

  if (invalidChannels.length > 0) {
    throw new Error(`The following channels are selected but not enabled: ${invalidChannels.join(', ')}`);
  }
  ...
  if (settings.quietHours) {
    ...
    if (!timeFormatRegex.test(settings.quietHours.start) || !timeFormatRegex.test(settings.quietHours.end)) {
  ...
  if (settings.reminderTiming) {
    ...
    for (const time of settings.reminderTiming) {
      if (!Number.isInteger(time) || time < 5 || time > 10080) {
        throw new Error('Reminder timing must be integers between 5 minutes and 7 days (10080 minutes)');
      }
    }
  }
}
